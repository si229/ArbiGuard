...
开仓节奏沿用之前 Go 版规则，核心目标是两边尽量同步成交，避免裸仓：

1. 扫描进程只负责发现机会和排序，不直接决定最终成交价。
2. 开仓执行进程收到机会后，先通知对应交易所 ticker 进程订阅该币种。
3. 任意一边 WS ticker 更新时，开仓执行进程都会读取两边最新盘口。
4. 做多腿用卖一方向 `ask`，做空腿用买一方向 `bid`。
5. 单批成交量按两边盘口可成交名义额取较小值：

```text
本批开仓名义额 = min(目标剩余名义额, 多头 ask 可成交名义额, 空头 bid 可成交名义额)
```

如果交易所只返回最优挂单价和数量，就用最优一档计算；如果后续接入深度档位，则按多档加权均价计算。

单批价格使用本批可成交量的加权均价：

```text
多头开仓均价 = 吃掉 ask 档位后的加权均价
空头开仓均价 = 吃掉 bid 档位后的加权均价
```

模拟盘：

- 不真正向交易所下单。
- 但仍按 WS 盘口数量、加权均价、手续费、保证金和资金费字段生成成交。
- 本批能成交多少，就按本批成交量生成模拟持仓。

实盘：

- 开仓默认使用 `FOK`，因为单批量已经按盘口可成交量计算好；如果不能整批成交，就宁可整批不成交。
- HTTPS 下单失败时，本批作废，执行单回到 `waiting_ws_ticker`，等下一次 ticker 重新计算价格和数量。
- HTTP 提交成功后，订单进入 `awaiting_live_open_fill`，等待账户/订单成交回报。
- 收到成交回报后，更新 `confirmed_notional`、`pending_notional`、`remaining_notional` 和成交进度。
- 如果确认成交量未达到目标，状态转为 `partial_live_open_continue`，继续等下一次 ticker 计算下一批。
- 如果确认成交量达到目标，开仓单完成，取消本地订阅，并把持仓交给平仓执行进程。

单边异常保护：

- 如果只有一边成交，执行进程必须优先补齐另一边对冲腿。
- 补对冲时不再追求原始候选价格，而是以已经成交腿的开仓均价作为对冲基准。
- 两边未追平前，不应再扩大已经领先那一边的仓位。
...
1. 单边爆仓保护触发。
2. 下架/停牌风险，且当前有盈利。
3. 当前实际净浮盈达到 `min_execution_profit_usdt`，立即平仓；这里的净浮盈必须已经扣除预估平仓手续费。
