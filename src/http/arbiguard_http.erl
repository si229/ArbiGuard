...
admin_html() ->
    unicode:characters_to_binary([
        ...
        "<section class='panel'><h2>Paper Positions</h2><div class='table-wrap'><table><thead><tr><th>Symbol</th><th>Long</th><th>Short</th><th>Notional</th><th>Long Entry</th><th>Long Current</th><th>Short Entry</th><th>Short Current</th><th>Unrealized</th><th>Expected</th><th>Funding Rounds</th><th>Funding PNL</th><th>Created</th></tr></thead><tbody id='positionRows'></tbody></table></div></section>",
        ...
        "function renderPositions(state){$('positionRows').innerHTML=(state.positions||[]).map(p=>{const lr=n(p.long_funding_settlement_count),sr=n(p.short_funding_settlement_count),rounds=lr+sr;return `<tr><td>${esc(p.symbol)}</td><td>${esc(p.long_exchange)}</td><td>${esc(p.short_exchange)}</td><td>${money(p.notional_usdt||p.notional)}</td><td>${esc(p.long_entry_price??'-')}</td><td>${esc(p.long_current_price??'-')}</td><td>${esc(p.short_entry_price??'-')}</td><td>${esc(p.short_current_price??'-')}</td><td class='${cls(p.unrealized_pnl)}'>${money(p.unrealized_pnl)}</td><td class='${cls(p.expected_net_profit)}'>${money(p.expected_net_profit)}</td><td>${rounds} (${lr}/${sr})</td><td class='${cls(p.funding_pnl)}'>${money(p.funding_pnl)}</td><td>${esc(p.opened_at??'-')}</td></tr>`}).join('')||'<tr><td colspan=13>No positions.</td></tr>'}",
