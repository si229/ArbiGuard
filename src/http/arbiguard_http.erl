...
-compile({nowarn_unused_function, [index_html/0]}).
...
admin_html() ->
    unicode:characters_to_binary([
        ...
        "<div><label>Capital USDT</label><input id='capital_usdt' value='10000'></div><div><label>Notional USDT</label><input id='execution_notional_usdt' value='200'></div>",
        ...
        "<section class='panel'><h2>Processes / ETS</h2><div class='table-wrap'><table><thead><tr><th>Exchange</th><th>WS Enabled</th><th>WS Connected</th><th>WS Status</th><th>Subs</th><th>Funding Rows</th><th>Last Refresh</th><th>Error</th></tr></thead><tbody id='processRows'></tbody></table></div></section>",
        ...
        "function renderProcesses(proc){$('processRows').innerHTML=(proc.exchanges||[]).map(e=>{const t=e.ticker||{},f=e.funding||{};return `<tr><td>${esc(e.exchange)}</td><td>${esc(t.ws_enabled)}</td><td class='${t.ws_connected?'pos':'warn'}'>${esc(t.ws_connected)}</td><td>${esc(t.ws_status||'-')}</td><td>${(t.subscriptions||[]).length}</td><td>${esc(f.last_count??'-')}</td><td>${esc(f.last_refresh_ms??'-')}</td><td>${esc(t.ws_error||f.last_error||'-')}</td></tr>`}).join('')||'<tr><td colspan=8>No process status.</td></tr>'}",
